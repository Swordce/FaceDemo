...
import com.facepp.library.bean.SearchFaceData;
...
public class OpenglActivity extends Activity
    ...
    private void init() {
        ...
        ivPreview = (ImageView) findViewById(R.id.opengl_image) ;
            ...
            public void run() {
                ...
                if (faces != null) {
                    if(!isFacing) {
                        ...
                        isFacing = true;
                        result = "";
                        byte[] img = getArray(imgData,camera);
                        CommonOperate commonOperate = new CommonOperate(Util.API_KEY,Util.API_SECRET,false);
                        try {
                            Response res = commonOperate.searchByOuterId(null,null,img,"test",1);
    ...
    private byte[] getArray(byte[] yuv,Camera camera){
        ...
        if (format == ImageFormat.NV21 || format == ImageFormat.YUY2 || format == ImageFormat.NV16) {
            ...
            // Get the YuV image
            YuvImage yuvImage = new YuvImage(yuv, format, w, h, null);
            // Convert YuV to Jpeg
    ...
    Handler timeHandle = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 10:
                    String message = "";
                    if(!TextUtils.isEmpty(result) && !result.contains("error_message")) {
                        Log.e("aaa",result);
                        SearchFaceData data = new Gson().fromJson(result,SearchFaceData.class);
                        if(data != null && data.getFaces().size() > 0) {
                            for (int i = 0; i < data.getResults().size(); i++) {
                                message = data.getResults().get(i).getFace_token()+",";
                            }
